...
type Scheduler interface {
	...
	//
	// 因此请确保先使用 SetRedisProviderStandalone 或
	//
	// SetRedisProviderCluster 或 SetRedisProviderFailOver 实例化redis连接
	...
	//
	// 这是一个一次性任务，不会重复执行
	RunOnceTimeAfter(delay time.Duration) (cancel CancelFunc)
	// RunAt 在某一时刻执行
	//
	//重复地在某个时间点执行任务
	//
...
func generateJobNameKey(name string) string {
	return fmt.Sprintf("go-sail:task-schedule-locker:%s", Base64Encode([]byte(name)))
}
...
func (j *taskJob) WithoutOverlapping() Scheduler {
	if lockDriver.client == nil && lockDriver.clusterClient == nil {
		panic("None available redis client")
	}
